...
# Por defecto, comienza el juego la maquina y con un nivel medio. profundidad = 2
def alphabeta_search(state, game, d=2, cutoff_test=None, eval_fn=None, player='X', dificultad=2):
    """Search game to determine best action; use alpha-beta runing.
    ...
    def max_value(state, alpha, beta, depth):
        if cutoff_test(state, depth):
            return eval_fn(state, player, dificultad) # Pasamos player y dificultad para usar en la heuristica.
        v = -infinity
        for (a, s) in game.successors(state):
